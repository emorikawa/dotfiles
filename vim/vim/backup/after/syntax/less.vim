" Language:	   Colored less Color Preview
" Maintainer:	Niklas Hofer <niklas+vim@lanpartei.de>
" URL:		   svn://lanpartei.de/vimrc/after/syntax/less.vim
...
function! s:PreviewlessColorInLine(where)
   " TODO use lessColor matchdata
...
if has("gui_running") || &t_Co==256
   " HACK modify lessDefinition to add @lessColors to its contains
   redir => s:olddef
      silent!  syn list lessDefinition
   redir END
   if s:olddef != ''
      let s:b = strridx(s:olddef,'matchgroup')
      if s:b != -1
         exe 'syn region lessDefinition '.strpart(s:olddef,s:b).',@lessColors'
      endif
   endif
   ...
   let i = 1
   while i <= line("$")
      call s:PreviewlessColorInLine(i)
      let i = i+1
   endwhile
   unlet i

   autocmd CursorHold * silent call s:PreviewlessColorInLine('.')
   autocmd CursorHoldI * silent call s:PreviewlessColorInLine('.')
   set ut=100
